function [Tout,wd,wd_dot,W] = getRealInput(mpcoutput,x0)
global quadrotorParameter
u = mpcoutput.uopt;
F = mpcoutput.F;
unext = mpcoutput.unext;
Fnext = mpcoutput.Fnext;
vnext = mpcoutput.vnext;
...
F = Fnext;
u = unext;
Omega_d  = [0 -wd(3) wd(2);wd(3) 0 -wd(1);-wd(2) wd(1) 0];
R = R+step*R*Omega_d;
R = R_normalized(R);
T = Tout;
v = vnext;
...
wy = -(pp(1)+qq(1)*W+R(1,2)*wx)/R(1,1);
wdd = [wx;wy;wz];
% wd_dot = (-(wdd-wd)/step-(wd-w)/step)/2;
wd_dot = (wd-wdd)/step;
end